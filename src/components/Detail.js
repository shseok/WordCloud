...
class Detail extends React.Component{
    ...
    _getText() { // 현재 보고있는 텍스트의 ID값에 해당하는 텍스트를 열겠다고 설정
    ...
    _getImage() {
        fetch(`${apiURL}/validate?textID=${this.props.match.params.textID}.json`).then(res => {
            if (res.status != 200) {
                throw new ErrorEvent(res.statusText);
            }
            return res.json();
    ...
    handleSubmit = () => {
        ...
        this.handleDialogToggle(); // 창 close
        if (!wordCloud.textID ||
            !wordCloud.text ||
            !wordCloud.maxCount ||
            !wordCloud.minLength ||
    ...
    handleValueChange = (e) => { // minLength, maxCount update
        let nextState = {};
        if (e.target.value % 1 === 0) { // 숫자인 경우
            if (e.target.valuue < 1) {
